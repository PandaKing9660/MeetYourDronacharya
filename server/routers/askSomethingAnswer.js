...
// call for deleting the answer
router.post ('/deleteAnswer', async (req, res) => {
  const {answerId} = req.body;
  try {
    // first find the answer that is needed to be deleted
...
// finding the title of the answer
router.post ('/fetchtitle', async (req, res) => {
  try {
    // searching the experience id
...
router.post ('/editAns', async (req, res) => {
  try {
    // finding the important information from request
    const {title, answer, tags, experienceId} = req.body;

    // updating the answer after finding the answer
    await askSomethingAnswer.findByIdAndUpdate (
      experienceId,
      {
        title: title,
